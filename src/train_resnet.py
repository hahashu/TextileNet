...
parser = argparse.ArgumentParser(
    description='PyTorch fashion textileNet resnet18 Training')
parser.add_argument('--partition', default='fibre', type=str, help='type of data')
...
train_loader = torch.utils.data.DataLoader(
    train_set, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, collate_fn=my_collate)
test_loader = torch.utils.data.DataLoader(
    test_set, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, collate_fn=my_collate)

# instantiate model

model = get_resnet18(num_classes=args.num_classes)
scripted_predictor = torch.jit.script(model).to(device)
...
if args.resume:
    ...
    assert os.path.isdir(
        f'./{DATA}_results/checkpoint_res'), 'Error: no checkpoint directory found!'
    checkpoint = torch.load(f'./{DATA}_results/checkpoint_res/ckpt.pth')
...
# change lr from high to nearly 1 schedully
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir(f'./{DATA}_results/checkpoint_res'):
            os.mkdir(f'./{DATA}_results/checkpoint_res')
        torch.save(state, f'./{DATA}_results/checkpoint_res/ckpt.pth')
        best_acc = acc
...
with open(f'./{DATA}_results/top1.txt', 'w') as f:
    for item in epoch_wise_top1s:
        f.write("%s\n" % item)

with open(f'./{DATA}_results/top3.txt', 'w') as f:
    for item in epoch_wise_top3s:
        f.write("%s\n" % item)

with open(f'./{DATA}_results/top5.txt', 'w') as f:
    for item in epoch_wise_top5s:
        f.write("%s\n" % item)


with open(f'./{DATA}_results/accres.png', 'w') as f:
    import matplotlib.pyplot as plt
